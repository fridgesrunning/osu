           ...
           if (Math.Max(prevVelocity, currVelocity) != 0)
            ...
            if (Math.Max(osuCurrObj.StrainTime, osuLastObj.StrainTime) < 1.25 * Math.Min(osuCurrObj.StrainTime, osuLastObj.StrainTime)) // If rhythms are the same.
            {
                if (osuCurrObj.Angle != null && osuLastObj.Angle != null && osuLastLastObj.Angle != null)
                {
                    ...
                    acuteAngleBonus *= (0.5 - Math.Min(0.25, Math.Sqrt(angleBonus) / (8 * Math.Sqrt(velocityChangeBonus + 1)))) + (0.5 + Math.Min(0.25, Math.Sqrt(angleBonus) / (8 * Math.Sqrt(velocityChangeBonus + 1)))) * (1 - Math.Min(acuteAngleBonus, Math.Pow(calcAcuteAngleBonus(lastAngle), 3)));
